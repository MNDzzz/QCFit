    ...
    <nav class="fixed top-0 left-0 w-full z-50 bg-slate-950/80 backdrop-blur-md border-b border-slate-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    <router-link to="/explore" class="text-stone-300 hover:text-white font-sans text-sm font-medium transition-colors">
                        Explore
                    </router-link>
                    <router-link to="/brands" class="text-stone-300 hover:text-white font-sans text-sm font-medium transition-colors">
                        Brands
                    </router-link>
                    <router-link to="/app/canvas" class="text-stone-300 hover:text-white font-sans text-sm font-medium transition-colors">
                        Studio
                    </router-link>
                </div>
                ...
                <div class="hidden md:flex items-center gap-4">
                    ...
                    <template v-if="!store.authenticated">
                        <router-link to="/login" class="text-stone-300 hover:text-white text-sm font-medium">
                            Log in
                        </router-link>
                        <router-link to="/register" class="bg-violet-600 hover:bg-violet-500 text-white px-4 py-2 rounded-full text-sm font-bold shadow-lg shadow-violet-500/20 transition-all transform hover:scale-105">
                            Join Free
                        </router-link>
                    </template>
                    <template v-else>
                         <router-link :to="`/u/${store.user.id}`" class="flex items-center gap-2">
        ...
        <div v-show="isOpen" class="md:hidden bg-slate-900 border-b border-slate-800">
             <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                <router-link to="/explore" class="block px-3 py-2 rounded-md text-base font-medium text-stone-300 hover:text-white hover:bg-slate-800">Explore</router-link>
                <router-link to="/app/canvas" class="block px-3 py-2 rounded-md text-base font-medium text-stone-300 hover:text-white hover:bg-slate-800">Studio</router-link>
             </div>
        </div>
    </nav>
</template>
